...
void proximity(int index, std::vector<int>& cluster,const std::vector<std::vector<float>> points, KdTree* tree, float distanceTol,std::vector<bool>isProcessed){
  ...
  //	std::cout << std::endl;
	...
	{
		if(!isProcessed[i])
			proximity(i,cluster,points,tree,distanceTol,isProcessed);
	}
}
